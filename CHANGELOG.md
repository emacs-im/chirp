...
- XChat text, reply, and reaction sends now use the official XDK for native encryption and signing and submit the same fixed GraphQL mutation without automatic retry or optimistic insertion. Reply targets remain view-local Appkit composer context, failed replies retain their text and target, validated acknowledgements clear the owned draft, and acknowledged reaction events enter canonical state before verified decryption updates their target chip.
- XChat now uses one Appkit application-owned `emacs-websocket` connection for realtime binary-Thrift delivery. Exact connection generations fence callbacks, official keepalives preserve the session, transport closure obtains a fresh short-lived token before reconnecting, transient typing activity stays outside persistent message state, and incoming events update canonical inbox and conversation projections without read acknowledgments or resubmitting already verified ciphertext. The authenticated profile supplies the current sender's label and avatar when direct-conversation metadata lists only the peer, new-group key events reach the native session before its composer can encrypt a message, and verified reaction operations become compact Appkit chips beneath their exact target message with a distinct chosen style for the current account.
...
- Thread translations now appear as soon as the request completes, without requiring a refresh.
- Returning from a dedicated video reader no longer reprints an unchanged source layout, preserving inline playback, pause, and mute state. Chirp now uses Appkit's shared canonical-window geometry observation and width measurement.
...
- Media layout planning now lives in `chirp-media-layout.el`. Normal non-condensed posts—including timelines, reposts, thread focuses, and replies—share one large SVG media renderer. A single item keeps its natural ratio within the full reference width and bounded media height; multiple items use X Web's current carousel algorithm with aspect-derived common height, 80-percent item-width caps, 4-pixel gutters, and center-cover boxes. Quoted posts and other condensed cards retain `TweetPhotos` cover grids. Appkit composites each sliced row into one mapped SVG display object and draws its shared translucent circle-and-triangle marker over video-like items; Arrow or Tab navigation moves the SVG `viewBox` without wrapping media or shifting surrounding text. Media viewer state and modes now live in `chirp-media-view.el`, and `C-x C-j` in `Chirp-Image` opens Dired at the exact rendered cache file.
