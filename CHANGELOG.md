...
- XChat now uses one Appkit application-owned `emacs-websocket` connection for realtime binary-Thrift delivery. Exact connection generations fence callbacks, official keepalives preserve the session, transport closure obtains a fresh short-lived token before reconnecting, and incoming events update canonical inbox and conversation projections without read acknowledgments.
