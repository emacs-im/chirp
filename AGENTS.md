...
- Chirp is an Emacs UI for X/Twitter. `chirp-x.el` owns explicit session credentials, `url.el` HTTP account transport, the direct `emacs-websocket` XChat connection boundary, persisted GraphQL construction, allowlisted REST roots, and X error decoding; do not add direct X calls outside that module. Authenticated POST retrievals disable `url.el` replay, become lifecycle-owned before quits can escape, and report every post-dispatch failure as an unknown remote outcome. XChat websocket tokens are short-lived, must never be persisted or logged, and may reach only the exact `wss://chat-ws.x.com/ws` endpoint. Its bounded public query-ID registry refresh may use Plz/curl, but must never receive X credentials or issue X API requests.
- `chirp.el` is the public entry point. External users load `(require 'chirp)`.
- `chirp-backend.el` owns operation selection, typed compose and upload orchestration, response adaptation, and normalized callback contracts. Chirp has no CLI transport; never add subprocess-backed X API behavior.
- `chirp-xchat.el` owns modern XChat variables, cursor validation, bounded HTTP and websocket Thrift decoding, official keepalive framing, and DM wire normalization.
- XChat media follows a strict boundary: the native module validates and encrypts local bytes into temporary ciphertext, `chirp-xchat-native.el` validates only cryptographic and file staging facts, and `chirp-x.el` generates transport UUIDs and performs GraphQL initialize, bounded TON-part upload, and GraphQL finalize. GraphQL uses the normal authenticated X web headers and one process-local `X-Client-UUID`; TON uses session cookies and CSRF only, without the public bearer or client UUID.
...
- `native/chirp-xchat-module` is the optional boundary for official-XDK cryptography. It owns native private and versioned conversation keys, verification, message and downloaded-media decryption, bounded local-media stream encryption, temporary ciphertext staging, and outgoing encryption/signing; it must not call X APIs, return key material to Lisp, persist plaintext, or load merely because Chirp loaded. X media transport is owned by `chirp-x.el`; inbound decryption selects only a verified key version previously retained from that conversation's authenticated event batch, and outgoing staging uses the latest verified version. Recovery binds the registered X user identity to the native session, and per-message Lisp input must never choose or override the signing sender. Lisp may receive only verified domain data, encrypted staging metadata, and opaque outgoing envelopes. Users configure its absolute path explicitly through `chirp-xchat-native-module-file`; never infer it from source trees, build directories, or `load-path`. Only the pinned official SDK may contact strictly validated HTTPS Juicebox realms, and only after an explicit PIN submission. Native workers own only Rust values and never retain an `emacs_env`, Lisp value, reference, or callback. Explicit destruction is the authoritative lifecycle end; GC finalization may request cancellation and detach but must never block Emacs.
- `chirp-render.el` renders normalized data. `chirp-media.el` owns cache paths, trusted public image acquisition, authenticated XChat attachment acquisition and plaintext cache persistence, thumbnail extraction, prefetching, and large-media display while Appkit owns bounded task scheduling and lifecycle cancellation.
- View modules orchestrate fetching and rendering; they must not duplicate backend, protocol, normalization, or media behavior. `chirp-dm.el` is the public unlock and inbox entry point. `chirp-dm-live.el` owns the one Appkit application-scoped websocket lifecycle, exact connection fencing, reconnect, and canonical live-event dispatch; it does not decode wire data or retain XChat tokens. `chirp-dm-state.el` owns session-scoped canonical conversation facts and cross-view invalidation, `chirp-dm-inbox.el` owns Appkit directory projection and inbox pagination, `chirp-dm-conversation.el` owns history, decryption orchestration, and the composer lifecycle, and `chirp-dm-render.el` owns normalized event projection and presentation. The DM subsystem does not persist decrypted message text or maintain parallel timeline/input state; decrypted attachment files belong only to the documented media cache. `chirp-actions.el` owns tweet compose and write actions, which share one backend request path.
