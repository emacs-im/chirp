...
- View modules orchestrate fetching and rendering; they must not duplicate backend, protocol, normalization, or media behavior. `chirp-dm.el` is the public unlock and inbox entry point. `chirp-dm-state.el` owns session-scoped canonical conversation facts and cross-view invalidation, `chirp-dm-inbox.el` owns Appkit directory projection and inbox pagination, `chirp-dm-conversation.el` owns history, decryption orchestration, and the composer lifecycle, and `chirp-dm-render.el` owns normalized event projection and presentation. The DM subsystem does not parse XChat wire data, persist decrypted message text, or maintain parallel timeline/input state; decrypted attachment files belong only to the documented media cache. `chirp-actions.el` owns tweet compose and write actions, which share one backend request path.
