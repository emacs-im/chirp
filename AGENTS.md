...
- `native/chirp-xchat-module` is the optional boundary for official-XDK cryptography. It owns native private and versioned conversation keys, verification, message and downloaded-media decryption, and outgoing encryption/signing; it must not call X APIs, return key material to Lisp, persist secrets, or load merely because Chirp loaded. X media ciphertext is retrieved by `chirp-x.el`, and native media decryption selects only a verified key version previously retained from that conversation's authenticated event batch. Recovery binds the registered X user identity to the native session, and per-message Lisp input must never choose or override the signing sender. Lisp may receive only verified domain data and opaque outgoing envelopes. Users configure its absolute path explicitly through `chirp-xchat-native-module-file`; never infer it from source trees, build directories, or `load-path`. Only the pinned official SDK may contact strictly validated HTTPS Juicebox realms, and only after an explicit PIN submission. Native workers own only Rust values and never retain an `emacs_env`, Lisp value, reference, or callback. Explicit destruction is the authoritative lifecycle end; GC finalization may request cancellation and detach but must never block Emacs.
