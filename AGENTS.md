...
- `chirp-backend.el` owns operation selection, typed compose/upload orchestration, response adaptation, and normalized callback contracts. `chirp-xchat.el` owns modern XChat variables, cursor validation, bounded HTTP and websocket Thrift decoding, official keepalive framing, and DM wire normalization. XChat local attachment bytes are encrypted by the native module into temporary ciphertext; only `chirp-x.el` may upload that ciphertext through the official Chat media API. Chirp has no CLI transport; never add subprocess-backed X API behavior.
